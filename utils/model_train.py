...
class CRMapDetector(Chain):
    ...
    def __call__(self, voxel_features, voxel_coords, pos_equal_one, neg_equal_one, targets):
        rm, psm = self.net(voxel_features, voxel_coords)

        p_pos = F.sigmoid(psm.transpose(0, 2, 3, 1))
        rm = rm.transpose(0, 2, 3, 1)
        rm = rm.reshape(rm.shape[:2] + (-1, 7))
        targets = targets.reshape(targets.shape[:2] + (-1, 7))
        ...
        reg_loss = F.huber_loss(rm_pos, targets_pos, 1)
        reg_loss = reg_loss / (F.sum(pos_equal_one) + 1e-6)
